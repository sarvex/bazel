...
public abstract class BazelLockFileValue implements SkyValue {
  ...
  public static BazelLockFileValue create(
    ...
    return new AutoValue_BazelLockFileValue(lockFileVersion, moduleFileHash, flags, localOverrideHashes, moduleDepGraph);
  }
  ...
  public ArrayList<String> getDiffLockfile(String moduleFileHash,
      ImmutableMap<String, String> localOverrideHashes, BzlmodFlagsAndEnvVars flags) {
    ...
    for (Map.Entry<String, String> entry : localOverrideHashes.entrySet()) {
      ...
      if (lockfileValue != null && !currentValue.equals(lockfileValue)) {
        diffLockfile.add("The MODULE.bazel file has changed for the overriden module: " + entry.getKey());
      }
    }
