...
class BazelLockfileTest(test_base.TestBase):
  ...
  def testLocalOverrideWithErrorMode(self):
    ...
    # Run with updated module and a different flag
    self.ScratchFile('bar/MODULE.bazel',
      [
        'module(name="bar")',
        'bazel_dep(name="hmmm")',
      ])
    ...
    self.assertIn(
      (
        'ERROR: Error computing the main repository mapping: Lock file is no'
        ' longer up-to-date because: The MODULE.bazel file has changed for the'
        ' overriden module: bar'
      ),
      stderr,
    )
